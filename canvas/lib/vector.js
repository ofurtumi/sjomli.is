...
class Vector {
	...
	crossProduct({ components }) {
		return new Vector(
			this.components[1] * components[2] -
				this.components[2] * components[1],
			this.components[2] * components[0] -
				this.components[0] * components[2],
	...
	transform(matrix) {
		...
		const multiplied = columns.map((column, i) =>
			column.map((c) => c * this.components[i])
		);
